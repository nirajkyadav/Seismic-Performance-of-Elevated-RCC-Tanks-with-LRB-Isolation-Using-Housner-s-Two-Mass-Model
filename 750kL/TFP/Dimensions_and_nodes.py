...
TFP_inclusion = 0            # 1 for TFP, 0 for fixed base

print("# ---------------------------------------------------")
print("# Rigid Diaphragm ON.")
if TFP_inclusion == 1:
    print("# TFP is installed.")
else:
    print("# TFP is NOT installed.")
print("# ---------------------------------------------------")
...
# TFP nodes ------------------------------------------------------------
if TFP_inclusion == 1:
    ops.node(1001, 3735.0, 0.0, 0.0)  
    ops.node(1002, 3735.0 * np.cos(60*np.pi/180),  3735.0 * np.sin(60*np.pi/180),  0.0)
    ops.node(1003, 3735.0 * np.cos(120*np.pi/180), 3735.0 * np.sin(120*np.pi/180), 0.0)
    ops.node(1004, 3735.0 * np.cos(180*np.pi/180), 3735.0 * np.sin(180*np.pi/180), 0.0)
    ops.node(1005, 3735.0 * np.cos(240*np.pi/180), 3735.0 * np.sin(240*np.pi/180), 0.0)
    ops.node(1006, 3735.0 * np.cos(300*np.pi/180), 3735.0 * np.sin(300*np.pi/180), 0.0)

    TFP_nodes = [1001, 1002, 1003, 1004, 1005, 1006]
...
if TFP_inclusion == 1:
    for node in TFP_nodes:
        ops.fix(node, 1, 1, 1, 1, 1, 1)

    Base_nodes = TFP_nodes
else:
